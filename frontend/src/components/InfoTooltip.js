...
export default function InfoTooltip({ isOpen, onClose, isRight }) {
  ...
  return (
    <section className={isOpen ? "popup popup_opened" : "popup"}>
      {isRight ? (
        <div className="alert">
          <button type="button" className="alert__button alert__button_close">
            <img
              src={closeIcon}
          ...
          <h1 className="alert__text">Vitória! Você se registrou.</h1>
        </div>
      ) : (
        <div className="alert">
          <button type="button" className="alert__button alert__button_close">
